...
public class FixedTimeSlot{
	Process[] mainProcess;
	boolean priority = true, repeat = true; //
	int[] queue;
	int fixedTime;
	
	//note: algorithm: {1:RR, 2:FCFS, 3:SJF, 4:SRTF, 5:Prio, 6:NPrio}
	public FixedTimeSlot() {}
	
	public FixedTimeSlot(Process[] process, boolean priority, int[] algorithm, int fixedTime) {
		...
		while(repeat) {
			...
			for(int i = 1; i <= algorithm.length; i++) {
				getProcess(algorithm[i-1], mainProcess, i);
			}
		}				
	}
	
	public void getProcess(int num, Process[] process, int queueNumber) {
		switch (num) {
		case 1: 
			RoundRobin(process, queueNumber);
			break;
		case 2:
	...
	public void RoundRobin(Process[] process, int queueNumber) {
		...
		for(int i = 0; i < roundRobinProcess.length; i++) {
			...
			for(int j = 0; j < burstTime; j++) {
				if(counter < fixedTime && queue[i] == queueNumber) {
					counter++;
...
Process[] nPrioProcess = SchedulingAlgorithmsUtilities.quicksort(process, 0, process.length - 1, 0);
	...
	public static void main(String[] args) {
		...
		FixedTimeSlot fts = new FixedTimeSlot(p, true, algo, 5);
