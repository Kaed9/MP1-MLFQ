...
public class mlfq{
	...
	public mlfq(Process[] process, boolean priority, int[] algorithm, int[] quantumTime) {
		...
		//RoundRobin(process, 4, 1);
		//RoundRobin(process, 5, 2);
	/*
	 1. set up algorithm
	 2. arrange arriving processes based on their arrival time
	 3. execute mlfq: 
				a.run first algorithm
				 	if: burst time exceeds quantum time
				 		then: demote the process
				b. continue till it finishes all process
...
Process[] prioProcess = SchedulingAlgorithmsUtilities.quicksort(process, 0, process.length - 1, 0);
	...
	public void NPrio(Process[] process, int queuNumber) {
Process[] nPrioProcess = SchedulingAlgorithmsUtilities.quicksort(process, 0, process.length - 1, 0);
	...
	public static void main(String[] args) {
		...
		int algo[] = {1, 1, 2};
		int quantumTime[] = {4, 5, 0};
		
		mlfq m = new mlfq(p, true, algo, quantumTime);
